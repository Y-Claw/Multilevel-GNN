...
class PathwayMessagePassing(MessagePassing):
    def __init__(self, aggr='softmax',
                 t=1.0, learn_t=False,
                 p=1.0, learn_p=False,
                 y=0.0, learn_y=False):
        ...
        if aggr in ['softmax_sg', 'softmax', 'softmax_sum']:
            ...
            super(PathwayMessagePassing, self).__init__(aggr=None)
            self.aggr = aggr
            ...
            super(PathwayMessagePassing, self).__init__(aggr=aggr)

    def aggregate(self, inputs, index, ptr=None, dim_size=None):

        if self.aggr in ['add', 'mean', 'max', None]:
            return super(PathwayMessagePassing, self).aggregate(inputs, index, ptr, dim_size)
