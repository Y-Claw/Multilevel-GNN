...
class PathwayHeadSeq(nn.Module):

    def __init__(self, args):
        super(PathwayHeadSeq, self).__init__()
    ...
    def forward(self, x, age=None):
        for i, layer in enumerate(self.conv_model):
            x = layer(x)
        if not self.args.only_mrna_pred:
            x = self.pooling(x)
            if self.pca_compare:
...
class MultilevelGNNSeq(nn.Module):

    def __init__(self, args, pca_params=None, pathway_indexs=None):
        super(MultilevelGNNSeq, self).__init__()
        self.pca_compare = args.pca_compare
        ...
        self.pathwayhead = PathwayHeadSeq(args)
        self.init_weight()
    ...
    def forward(self, input_batch, x=None, gene_pca_match=None, raw_indice=None, age=None, require_grad=True):
        ...
        if self.args.use_age:
            x = self.pathwayhead(x, age)
        else:
            x = self.pathwayhead(x)
    ...
    def load_ckpt(self, state_dict):
        self.learnable_pca_params = nn.Parameter(data=torch.zeros(state_dict['learnable_pca_params'].shape), requires_grad=(not self.args.freeze_pca_weight))
        self.set_info_mask(state_dict['info_mask'])
        layer_names = [name for name, _ in self.named_parameters()]
        partial_state_dict = {k: v for k, v in state_dict.items() if k in layer_names}
